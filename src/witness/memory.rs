...
impl SafeMemory {
    ...
    fn write_short_positive(&mut self, store: &impl AsStoreRef, ptr: usize, fr: &BigInt) -> Result<()> {
    ...
    fn write_short_negative(&mut self,  store: &impl AsStoreRef, ptr: usize, fr: &BigInt) -> Result<()> {
        // 2s complement
    ...
    fn write_long_normal(&mut self,store: &impl AsStoreRef, ptr: usize, fr: &BigInt) -> Result<()> {
    ...
    pub fn read_big(&self,store: &impl AsStoreRef, ptr: usize, num_bytes: usize) -> Result<BigInt> {
        let view = self.view(store);
        let buf = unsafe { view.data_unchecked_mut() };
...
mod tests {
    ...
    fn read_write_fr(num: BigInt) {
        let (mut mem,store) = new();
        mem.write_fr(&store, 0, &num).unwrap();
